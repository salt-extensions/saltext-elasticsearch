...
Copied from original returner and modified to support elasticsearch 9.x
...
def returner(ret):
    ...
    if job_fun in STATE_FUNCTIONS:
        ...
        if options["states_order_output"] and isinstance(ret["return"], dict):
            ...
            for uid, data in list(ret["return"].items()):
        ...
        if options["states_count"]:
            for state_data in ret["return"].values():
                if not isinstance(state_data, dict) or "result" not in state_data:
                    continue
    ...
    # Post the payload
    __salt__["elasticsearch.document_create"](index=index, document=salt.utils.json.dumps(data))
